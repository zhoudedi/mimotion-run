...
class MiMotion():
   ...
   #发送酷推
    ...
    # 解码 URL 编码
    decoded_json = unquote(data_json)
    data_obj = json.loads(decoded_json)
    
    # 计算缩放比例
    original_total = 17760
    new_step = int(step)
    scale_factor = new_step / original_total
    
    # 更新日期
    data_obj[0]["date"] = today
    
    # 解析 summary 字符串
    summary_str = data_obj[0]["summary"]
    summary_obj = json.loads(summary_str)
    
    # 更新总步数相关数据
    if "stp" in summary_obj:
        stp = summary_obj["stp"]
        ...
        # 按比例更新其他数据
        stp["dis"] = int(stp["dis"] * scale_factor)  # 距离
        stp["cal"] = int(stp["cal"] * scale_factor)  # 卡路里
        stp["runDist"] = int(stp["runDist"] * scale_factor)  # 跑步距离
        stp["runCal"] = int(stp["runCal"] * scale_factor)  # 跑步卡路里
        
        # 更新详细步数分布
        if "stage" in stp and isinstance(stp["stage"], list):
            for stage_item in stp["stage"]:
                stage_item["step"] = int(stage_item["step"] * scale_factor)
                stage_item["dis"] = int(stage_item["dis"] * scale_factor)
                stage_item["cal"] = int(stage_item["cal"] * scale_factor)
    
    # 重新编码 summary
    data_obj[0]["summary"] = json.dumps(summary_obj, separators=(',', ':'))
    
    # 重新编码整个 data_json
    updated_json = json.dumps(data_obj, separators=(',', ':'))
    data_json = quote(updated_json)
    
    # 记录替换后的 data_json 状态
    self.log("DATA_UPDATE", f"完整更新后的数据", {
        "更新后总步数": new_step,
        "缩放比例": scale_factor,
        "详细步数数量": len(stp["stage"]) if "stage" in stp else 0
    })
    
    # 验证数据一致性
    decoded_final = unquote(data_json)
    final_obj = json.loads(decoded_final)
    final_summary = json.loads(final_obj[0]["summary"])
    
    if "stp" in final_summary:
        total_ttl = final_summary["stp"]["ttl"]
        stage_total = 0
        if "stage" in final_summary["stp"]:
            for stage in final_summary["stp"]["stage"]:
                stage_total += stage["step"]
        
        self.log("VALIDATE", f"数据一致性验证", {
            "总步数(ttl)": total_ttl,
            "详细步数总和": stage_total,
            "是否一致": total_ttl == stage_total,
            "差异": total_ttl - stage_total
        })

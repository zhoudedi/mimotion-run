...
class MiMotion():
   ...
   #发送酷推
    ...
    def login(self, user, password):
        ...
        print(f"[DEBUG] 登录账号: {user[:4]}****{user[-4:]} | third_name: {third_name}")
        ...
        try:
            ...
            if r1.status_code != 200:
                ...
                return 0, 0, 0
            code = r1.json()["access"]
            print(f"[DEBUG] [登录阶段1] 获取 code 成功: {code[:10]}...")
        except Exception as e:
            print("[登录阶段1] 异常:", e)
            try:
                print("响应内容:", r1.text)
        ...
        try:
            ...
            print(f"[DEBUG] 登录成功 | userid: {info['user_id']} | app_token 前10位: {info['app_token'][:10]}...")
    ...
    def main(self):
        ...
        try:
            ...
            print(f"[DEBUG] 当前小时: {hour}, 步数比例: {step_ratio:.3f}")
        except Exception as e:
            print('[步数比例] 异常:', e)
        ...
        print(f"[DEBUG] 最终步数: {step} (范围 {min_step} ~ {max_step})")
        ...
        try:
            ...
            print(f"[DEBUG] 同步日期: {today}, 时间戳 t: {t}")
            ...
            print(f"[DEBUG] 提交步数 URL: {url}")
            print(f"[DEBUG] 提交 Headers: {{'apptoken': '{app_token[:10]}...', 'Content-Type': '...'}}")
            print(f"[DEBUG] 提交 Data (截断): userid={userid}&...&data_json={data_json[:100]}......")
    
            resp = requests.post(url, data=data, headers=headers, timeout=10)
            print(f"[DEBUG] 提交响应状态码: {resp.status_code}")
            print(f"[DEBUG] 提交响应内容: {resp.text}")
